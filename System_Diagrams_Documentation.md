...
| **EnrollmentForm** | Uploaded enrollment documents | `file_path`, `status`, FK → Student |
...
> [!NOTE]
> One field in the CCA ERD is explicitly marked with **"AI"** labels because it is computed by our AI engine, not manually entered:
...
### Key Process: Student Self-Registration

> [!IMPORTANT]
> CCA has a **self-registration workflow** that HOP does not. Students fill out a form and upload a photo of their enrollment document. The account is created in an **inactive state** (`is_active = 0`). The **Registrar** then reviews the uploaded document, verifies the data, and activates the account. This prevents unauthorized access while reducing manual data entry.
...
> "The student's account is created with `is_active = 0` and enrollment status = `Pending Validation`. If they try to log in, the system returns a 403 error with the message 'Account pending Registrar verification.' The Registrar reviews their uploaded enrollment form, verifies the data, and clicks 'Verify' — which sets `is_active = 1` and `enrollment_status = Enrolled`. Only then can the student access the system."
