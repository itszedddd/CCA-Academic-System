...
export default function LoginPage({ onLogin, isDarkMode, setIsDarkMode, onNavigatePreRegister }) {
  ...
  return (
    <div className={`flex flex-col lg:flex-row min-h-screen w-full transition-colors duration-700 font-sans ${isDarkMode ? 'bg-slate-900' : 'bg-slate-50'}`}>
      ...
      <div className="lg:w-[45%] w-full bg-[#022868] flex flex-col items-center justify-center p-12 lg:min-h-screen min-h-[30vh] relative overflow-hidden text-center order-1">
        <div className="max-w-md w-full z-10 animate-fade-in-down flex items-center justify-center">
          <img src="/cca-logo-v1.png" alt="CCA Logo" className="w-full h-auto object-contain max-h-[80vh]" />
        </div>
      </div>
      ...
      <div className={`lg:w-[50%] w-full flex-col items-center justify-center p-6 sm:p-12 relative order-1 lg:order-2 ${mobileTab === 'login' ? 'flex' : 'hidden lg:flex'}`}>
        ...
        <button 
          ...
          className={`absolute top-6 right-6 p-2.5 rounded-full backdrop-blur-md transition-all z-10 shadow-sm border ${isDarkMode ? 'bg-slate-800 hover:bg-slate-700 text-white border-slate-700' : 'bg-white hover:bg-slate-50 text-slate-600 border-slate-200 shadow-sm'}`}
        >
          {isDarkMode ? (
        ...
        <div className={`w-full max-w-md`}>
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] transition-colors animate-zoom-in">
            ...
            {successMsg && (
              <div className="bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/50 text-green-700 dark:text-green-400 text-sm py-3 px-4 rounded-xl flex items-center mb-6 font-bold">
            ...
            {error && (
              <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/50 text-red-600 dark:text-red-500 text-sm py-3 px-4 rounded-xl flex items-center animate-shake mb-6">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <svg className="h-5 w-5 text-slate-400 dark:text-slate-500 group-focus-within:text-[#022868] dark:group-focus-within:text-blue-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                  </div>
                  <input type="text" required className="block w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 outline-none focus:ring-2 focus:ring-[#022868]/50 transition-all font-medium" placeholder="admin" value={username} onChange={(e) => setUsername(e.target.value)} />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input type={showPassword ? 'text' : 'password'} required className="block w-full pl-11 pr-12 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-[#022868]/50 transition-all font-medium" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} />
              ...
              <button type="submit" disabled={loading} className="w-full relative py-4 bg-[#022868] hover:bg-[#011a45] text-white font-bold tracking-widest rounded-xl shadow-[0_10px_20px_-5px_rgba(2,40,104,0.4)] transition-all flex justify-center items-center mt-4">
                {loading ? (
