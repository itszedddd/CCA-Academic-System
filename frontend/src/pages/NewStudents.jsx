                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                  {gradeStudents.map(student => (
                    <tr key={student.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="p-4 font-mono text-sm font-bold text-slate-600 dark:text-slate-300">
                        #{student.id}
                      </td>
                      <td className="p-4">
                        <div className="font-bold text-slate-800 dark:text-white">{student.student_first_name} {student.student_last_name}</div>
                        <div className="text-xs text-slate-500">{student.birth_date}</div>
                      </td>
                      ...
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                          student.assessment_status === 'Passed' ? 'bg-green-100 text-green-700' : 
                          student.assessment_status === 'Failed' ? 'bg-red-100 text-red-700' : 
                      ...
                      <td className="p-4 flex gap-2">
                        <button 
                          ...
                          className="px-4 py-2 bg-brand-50 hover:bg-brand-100 text-brand-700 dark:bg-brand-900/30 dark:hover:bg-brand-900/50 dark:text-brand-300 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors"
                        >
                          Review
                        </button>
                        <button 
                          onClick={() => handleArchiveClick(student)}
                          className="px-4 py-2 bg-slate-100 hover:bg-red-50 text-slate-700 hover:text-red-700 dark:bg-slate-800 dark:hover:bg-red-900/30 dark:text-slate-300 dark:hover:text-red-400 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors"
                        >
                          Archive
                        </button>
                      </td>
