...
export default function Header({ activeTab, isDarkMode, setIsDarkMode, isAccessibleMode, setIsAccessibleMode, warnings, showNotifications, setShowNotifications, setActiveTab, searchQuery, setSearchQuery, showSearchResults, setShowSearchResults, searchResults, currentRole, onMenuToggle }) {
  ...
  return (
    <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 h-16 flex items-center justify-between px-4 md:px-8 z-30 print:hidden">
      <div className="flex items-center space-x-3 min-w-0">
        {/* Hamburger - mobile only */}
        <button
          onClick={onMenuToggle}
          className="md:hidden p-2 -ml-1 text-slate-500 hover:text-brand-600 dark:text-slate-400 dark:hover:text-white transition-colors rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700"
          id="mobile-menu-toggle"
        >
          <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
        </button>
        <h2 className="text-lg md:text-xl font-bold text-slate-800 dark:text-white truncate">{activeTab}</h2>
      </div>
      ...
      <div className="flex items-center space-x-2 md:space-x-3">
        ...
        <div className="relative hidden sm:block">
          ...
          <input
            ...
            className="w-40 lg:w-56 pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-600 rounded-full text-sm bg-slate-50 dark:bg-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all"
          />
